...
def main(cfg):
  ...
  for i in range(120):
    ...
    camera_orn = tf.Rotation.from_euler('xyz', [theta, np.pi/2, np.pi])
    camera_quat = camera_orn.as_quat() # wxyz
    camera_quat = np.concatenate((camera_quat[1:], camera_quat[[0]])) # xyzw
    digit_top.set_base_pose([-0.015, 0, 0.05], camera_quat) # 0.5 is the height of the DIGIT
